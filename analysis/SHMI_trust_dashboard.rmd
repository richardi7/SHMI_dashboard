...
conditions<-data_diagnostic %>% filter(ProviderCode==params$trust & Observed_Diagnosis>5 & (Expected_Diagnosis<-5|Expected_Diagnosis>5))%>% 
...
shmi_table<-shmi_table%>% rename(MeasureID='measure')
shmi_table$Shortname=trustname$Shortname
shmi_table<-shmi_table[,c(1,3,2)]

counts_table<-counts%>% filter(PeriodEnd==latest_publication) %>% select(measure_shortlabel,Shortname,value)
counts_table<-counts_table%>% rename(MeasureID='measure_shortlabel')
crude_rates_table<- crude_rates_longer%>%filter((OrganisationName ==trustname$OrganisationName) & PeriodEnd==latest_publication) %>% select(measure_shortlabel,Shortname,value)
crude_rates_table<-crude_rates_table%>% rename(MeasureID='measure_shortlabel')

pod<-pod%>%filter(PeriodEnd==latest_publication) %>% select(pod_label,Shortname,value)
pod<-pod%>% rename(MeasureID='pod_label')
pod$MeasureID=ifelse(substr(pod$MeasureID,1,9)=="Deaths in","Deaths_in",pod$MeasureID)
pod$MeasureID=ifelse(substr(pod$MeasureID,1,9)=="Deaths ou","Deaths_outside",pod$MeasureID)
pod$MeasureID=ifelse(substr(pod$MeasureID,1,6)=="Spells","Spells_palliative",pod$MeasureID)
pod$MeasureID=ifelse(substr(pod$MeasureID,1,8)=="Deaths w","Deaths_palliative",pod$MeasureID)

coding<-coding%>% filter(PeriodEnd==latest_publication) %>% select(coding_shortlabel,Shortname,value)
coding<-coding%>% rename(MeasureID='coding_shortlabel')
coding$MeasureID=ifelse(substr(coding$MeasureID,1,5)=="Inval","Invalid",coding$MeasureID)
coding$MeasureID=ifelse(substr(coding$MeasureID,1,5)=="Prima","symptom",coding$MeasureID)
coding$MeasureID=ifelse(grepl("non-elective",coding$MeasureID),"DepthNonElective",
                      ifelse(grepl("elective",coding$MeasureID),"DepthElective",coding$MeasureID))

table_data<-rbind(pod,shmi_table,crude_rates_table,counts_table,coding)

table_data$value=round(table_data$value,1)

table_data$Measure <- factor(
  table_data$MeasureID,
  ordered = TRUE,
  levels = c(
    "SHMI_VALUE",
    "SHMI_OD_LL",
...
england_data<-table_data[table_data$Shortname == "England", ]
england_data<-england_data[,c(1,3)]
england_data<-pivot_wider(england_data, names_from=MeasureID, values_from=value)
england_data$Invalid=ifelse(is.na(england_data$Invalid),0,england_data$Invalid)

trust_data<-table_data[table_data$Shortname != "England", ]
trust_data<-trust_data[,c(1,3)]
trust_data$value=ifelse(is.na(trust_data$value),0,trust_data$value)
trust_data<-pivot_wider(trust_data, names_from=MeasureID, values_from=value)
#trust_data<-trust_data%>% rename_at( 17, ~"Invalid" )
trust_data$Invalid=ifelse(is.na(trust_data$Invalid),0,trust_data$Invalid)


table_data<-table_data %>%  pivot_wider(names_from = Shortname, values_from = value)
table_data<-arrange(table_data,Measure)
table_data<-table_data[,c(2:4)]
...
kbl_shmi<-filter(table_data,table_data$Measure %in% c("SHMI", "SHMI lower limit","SHMI upper limit"))
kbl_shmi %>%
  kbl() %>%
  kable_styling(font_size = 12)
kbl_counts<-filter(table_data,table_data$Measure %in% c("Spells", 
                                    "Observed deaths","Expected deaths",
                                    "Excess deaths"))
kbl_counts %>%
  kbl() %>%
  kable_styling(font_size = 12)
kbl_rates<-filter(table_data,table_data$Measure %in% c("Crude death rate per 100 spells",
                                    "Non-elective crude mortality per 100 spells",
                                    "Elective crude mortality per 100 spells"))
kbl_rates %>%
  kbl() %>%
  kable_styling(font_size = 12)
kbl_coding<-filter(table_data,table_data$Measure %in% c("Depth of coding - Elective",
                                    "Depth of coding - Non Elective","Invalid primary diagnosis (%)",
                                    "Primary diagnosis symptom or sign (%)"))
kbl_coding %>%
  kbl() %>%
  kable_styling(font_size = 12)
kbl_pod<-filter(table_data,table_data$Measure %in% c( "Spells with palliative care (%)",
                                    "Deaths with palliative care (%)","Deaths in hospital (%)",
                                    "Deaths outside hospital (%)"))
kbl_pod %>%
  kbl() %>%
  kable_styling(font_size = 12)
...
### SHMI {data-height=400 data-width=300 .mobile}
...
### Excess deaths by condition, period up to `r latest_publication_text`. Click to see [Conditions over time]  {data-height=400 data-width=300 .mobile}

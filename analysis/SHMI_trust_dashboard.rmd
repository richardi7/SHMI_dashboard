...
shmi_text <-  shmi_text %>%
  ...
  mutate(previous_periodstart = lag(PeriodEnd,12, order_by =   PeriodEnd))

shmi_text <-  arrange(shmi_text, measure, PeriodEnd)

#create variable for latest period, values and band
...
<p><small>The SHMI for `r trustname$Shortname` for `r from_publication_text` to `r latest_publication_text` is `r round(trustname$value,3)`.
This `r hi_or_low` and is `r if(round(trustname$value,3)>round(shmi_textlatestSHMI$previous_value,3)){"higher (worse) than for"} else if(round(trustname$value,3)<round(shmi_textlatestSHMI$previous_value,3)){"lower (better) than for"} else {"same as"}` the previous period (`r format(shmi_textlatestSHMI$previous_periodstart,'%B %Y')` to `r format(shmi_textlatestSHMI$previous_period,'%B %Y') `) when it was `r round(shmi_textlatestSHMI$previous_value,3)`   and  `r if(shmi_textlatestBAND$previous_value==1) {"higher than expected"}  else if(shmi_textlatestBAND$previous_value==2) {"as expected"} else {"lower than expected"}`.
</small></p>
...
### Excess deaths by condition, period up to `r latest_publication_text`. Click to see [Conditions over time]  {data-height=400}
```{r Excess_deaths cht}
diag_excess_fig %>% 
  config(displayModeBar = F)
```

Row {.tabset .tabset-fade  .no-mobile}
-------------------------------------

### <span class="badge badge-primary">All trusts</span> {data-height=400  .no-mobile}
...
### <span class="badge badge-primary">Crude death rates</span> {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Activity</span> {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Relative activity changes</span> {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Conditions</span> {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Coding</span>  {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Palliative care</span>  {data-height=400 .no-mobile}
...
### <span class="badge badge-primary">Site</span>  {data-height=400 .no-mobile}
...
### Depth of coding: elective {.no-mobile}
...
### Depth of coding: non-elective {.no-mobile}
...
### Coding: signs and symptoms{.no-mobile}
...
### Place of death: in hospital {.no-mobile}
...
### Palliative care- deaths{.no-mobile}
...
### Palliative care- Spells{.no-mobile}
...
### Crude rate of deaths: all{.no-mobile}
...
### Crude rate of deaths: Non-elective{.no-mobile}
...
### Crude rate of deaths: elective{.no-mobile}
